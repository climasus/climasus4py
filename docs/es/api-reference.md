...
- `sus_pipeline(...)` -> ejecuta import, clean, standardize, filter, variables y aggregate.
...
- `groups=` en `sus_filter` requiere `climasus-data` local.
- `lang="en"` y `lang="es"` en `sus_standardize` tambien requieren diccionarios locales.
- `sus_census(..., census=None)` todavia no tiene autoload en Python.
